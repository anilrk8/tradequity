"""
LLM commentary module — uses local Ollama (Mistral) to generate plain-English
interpretations of seasonal analysis results.

No data leaves your machine. Requires Ollama running at localhost:11434.
Model: mistral:latest

To disable this feature entirely, set ENABLE_AI = False in app.py.
To revert: git checkout HEAD -- src/llm.py app.py requirements.txt
"""
...
def _check_ollama() -> tuple[bool, str]:
    """Return (available, error_message). Checks if Ollama is reachable."""
    try:
        import ollama
